...
short src_ln1 [] = {
	  0,  34,  34,  35,  36,  37,  38,  40, 
	 41,  42,  42,  43,  44,  45,  46,  47, 
	 48,  49,  41,  51,  41,  33,  52,  52, 
	  0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "list_c.pml", 1, 23 },
...
short src_ln0 [] = {
	  0,   9,  11,  15,  16,  17,  18,  20, 
	 21,  21,  22,  22,  20,  24,  20,  24, 
	 25,  14,  27,  27,  28,  28,   8,  30, 
	 30,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "list_c.pml", 1, 24 },
...
{	int j = 0, h = now._nr_pr;
	...
	case 2:	/* list_pop */
		...
		/* locals: */
#ifdef VAR_RANGES
		logval("list_pop:first_item", ((P2 *)pptr(h))->first_item);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 1:	/* list_remove */
		...
		/* locals: */
#ifdef VAR_RANGES
		logval("list_remove:item", ((P1 *)pptr(h))->item);
		logval("list_remove:prev", ((P1 *)pptr(h))->prev);
		logval("list_remove:curr", ((P1 *)pptr(h))->curr);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* list_add */
		...
		/* locals: */
#ifdef VAR_RANGES
		logval("list_add:item", ((P0 *)pptr(h))->item);
		logval("list_add:i", ((P0 *)pptr(h))->i);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
	...
	{	int l_in; /* 2 */
		for (l_in = 0; l_in < 10; l_in++)
		{
			logval("next[l_in]", now.next[l_in]);
		}
	}
...
{	/* int i; */
	...
	case 3:
		...
		break;
	case 2:
		printf("local vars proc %d (list_pop):\n", pid);
	printf("	int    first_item:	%d\n", ((P2 *)pptr(pid))->first_item);
		break;
	case 1:
		printf("local vars proc %d (list_remove):\n", pid);
	printf("	int    item:	%d\n", ((P1 *)pptr(pid))->item);
	printf("	int    prev:	%d\n", ((P1 *)pptr(pid))->prev);
	printf("	int    curr:	%d\n", ((P1 *)pptr(pid))->curr);
		break;
	case 0:
		printf("local vars proc %d (list_add):\n", pid);
	printf("	int    item:	%d\n", ((P0 *)pptr(pid))->item);
	printf("	int    i:	%d\n", ((P0 *)pptr(pid))->i);
		break;
	}
