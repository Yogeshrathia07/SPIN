...
{
		now.channel1 = addqueue(calling_pid, 1, 1);
#ifdef VAR_RANGES
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.channel1) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.channel1) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.channel1) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.channel1) )))
			    ) return 0; break;
...
{	/* int i; */
	...
	printf("	mtype  msg2:	2\n");
	printf("	mtype  err:	1\n");
	printf("	chan channel1 (=%d):	len %d:\t", now.channel1, q_len(now.channel1));
	c_chandump(now.channel1);
}
void
...
printm(int x, char *s)
{
	...
	case 2: Printf("msg2"); return;
	case 3: Printf("msg1"); return;
